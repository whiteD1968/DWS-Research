...
import { researchTypes, researchType } from "@/lib/research-organization";
...
export function ResearchTopicForm({ topic }: { topic?: Topic }) {
  return <form action={manageTopic} className="form-stack research-form">
    ...
    <label className="field">Research Area<input name="research_area" maxLength={160} defaultValue={typeof topic?.metadata.research_area === "string" ? topic.metadata.research_area : ""} placeholder="Additive Manufacturing" /></label>
    <label className="field">Research Type<select name="research_type" defaultValue={researchType(topic?.metadata)}>{researchTypes.map(type => <option key={type}>{type}</option>)}</select></label>
