...
import { BaseBoxShapeUtil, HTMLContainer, T, Tldraw, createShapeId, getSnapshot, loadSnapshot, DefaultColorStyle, DefaultFontStyle, defaultHandleExternalTldrawContent, renderPlaintextFromRichText, type Editor, type TLBaseShape, type TLStoreSnapshot, type TLRichText } from "tldraw";
...
type RecordShape = TLBaseShape<"research-record", { w: number; h: number; recordKey: string }>;
declare module "@tldraw/tlschema" { interface TLGlobalShapePropsMap { "research-record": { w: number; h: number; recordKey: string } } }
...
function RecordCard({ shape }: { shape: RecordShape }) {
  ...
  return <HTMLContainer className="research-board-card" style={{ width: shape.props.w, height: shape.props.h }}>
    {record?.image && <img src={record.image} alt="" loading="lazy" draggable={false} />}
...
class ResearchRecordUtil extends BaseBoxShapeUtil<RecordShape> {
  ...
  static override props = { w: T.number, h: T.number, recordKey: T.string };
...
export function ResearchBoardCanvas({ board, records: initialRecords, ownerId, persist = saveBoard }: BoardCanvasProps & { persist?: typeof saveBoard }) {
  ...
  function place(record: BoardRecord, position?: { x: number; y: number }) {
    ...
    if (!ed) return;
    const center = position || ed.getViewportPageBounds().center;
    ed.createShape<RecordShape>({ type: "research-record", x: center.x, y: center.y, props: { recordKey: record.key, w: 280, h: 300 } });
  }
  ...
  async function uploadImage(job: ImageJob) {
    try {
      ...
      setRecords(prev => [...prev.filter(r => r.key !== record.key), record]); place(record, job.point);
  ...
  function mount(ed: Editor) {
    editor.current = ed;
    try {
      if (board.snapshot) loadSnapshot(ed.store, { document: board.snapshot as TLStoreSnapshot });
      else if (board.metadata.composition && ed.getCurrentPageShapeIds().size === 0) {
        const composition = board.metadata.composition;
        const frames = composition.frames.map(() => createShapeId());
        ed.run(() => {
          composition.frames.forEach((f, i) => ed.createShape({ id: frames[i], type: "frame", x: f.x, y: f.y, props: { w: f.w, h: f.h, name: f.title } }));
          composition.placements.forEach(p => ed.createShape<RecordShape>({ type: "research-record", parentId: frames[p.frame], x: p.x, y: p.y, props: { recordKey: p.key, w: p.w, h: p.h } }));
        });
      }
    ...
    if (!board.snapshot && !stopped.current) { pending.current = getSnapshot(ed.store).document; void flush(); }
    ...
    return () => { unsubscribe(); stopNoteStyle(); clearTimeout(timeout); void flush(); window.removeEventListener("beforeunload", beforeUnload); editor.current = null; };
  }
  return <div className="board-workspace">
    <header className="board-header"><Link href={board.research_thread_id ? `/research/${board.research_thread_id}/boards` : "/boards"} onClick={e => { if ((pending.current || running.current) && !window.confirm("Changes are not saved. Leave this board?")) e.preventDefault(); }}>Research Topic</Link><button className="board-title" onClick={() => setDetails(!details)} title="Edit board details">{title}</button><span role="status">{status}</span>{status === "Error saving" && <button onClick={() => { stopped.current = false; void flush(); }}>Retry</button>}<button className="button" onClick={() => setPicker(!picker)}>+ Add</button>
