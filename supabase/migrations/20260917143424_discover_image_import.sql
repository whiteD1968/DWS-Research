-- Preserve distinct image identities while retaining source-page provenance.
-- Existing web imports keep title deduplication; no tables or policies change.
create or replace function public.import_discover_results(session_id uuid, result_ids text[], collection_id uuid default null, collection_title text default null, source_matches jsonb default '{}'::jsonb)
...
begin
  ...
  if coalesce(cardinality(result_ids), 0) not between 1 and 100 then raise exception 'Select 1 to 100 results'; end if;
  ...
  for item in select value from jsonb_array_elements(session_row.result_snapshot) where value->>'id' = any(result_ids) loop
    ...
    select r.id into reference_uuid from public."references" r where r.owner_id = uid and (
      ...
      (coalesce(item->>'resultType', '') <> 'image' and r.reference_type <> 'image' and
    ...
    if reference_uuid is null then
      provenance := jsonb_build_object('normalized_url', item->>'url', 'discover_session_id', session_id,
        'source_page_url', item->>'sourcePageUrl', 'accessed_at', now(), 'original_result_type', item->>'resultType', 'external_result', item);
